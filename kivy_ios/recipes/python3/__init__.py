...
class Python3Recipe(Recipe):
    ...
    def get_build_env(self, plat):
        ...
        build_env["PATH"] = "{}:{}".format(
            join(self.ctx.dist_dir, "hostpython3", "bin"), os.environ["PATH"]
        )
    ...
    def build_platform(self, plat):
        ...
        shprint(
            sh.make, self.ctx.concurrent_make, "CFLAGS={}".format(build_env["CFLAGS"])
        )
    ...
    def reduce_python(self):
        ...
        with cd(join(self.ctx.dist_dir, "root", "python3", "lib", "python3.11")):
            sh.rm(
                "-rf",
                "wsgiref",
                "curses",
